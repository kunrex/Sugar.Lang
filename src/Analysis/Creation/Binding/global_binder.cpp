...
namespace Analysis::Creation::Binding
{
    ...
    void CreateProperty(const IParseNode* const propertyNode, IUserDefinedType* const dataType)
    {
        ...
        {
            const auto getBody = getNode->GetChild(static_cast<int>(ChildCode::Body));
            const auto setBody = setNode->GetChild(static_cast<int>(ChildCode::Body));

            if (getBody != nullptr && setBody != nullptr)
                CreateGetSetProperty(propertyNode, value, dataType);
            else  if (getBody == nullptr && setBody == nullptr)
        ...
        {
            if (const auto getBody = getNode->GetChild(static_cast<int>(ChildCode::Body)); getBody == nullptr)
        ...
        {
            if (const auto setBody = setNode->GetChild(static_cast<int>(ChildCode::Body)); setBody == nullptr)
    ...
    std::tuple<MethodFunction*, VoidDefinition*> CreateIndexerFunctions(const IParseNode* const getNode, const IParseNode* const setNode, const IParseNode* declarationNode, const std::vector<const IDataType*>& parameters, const IDataType* const creationType, const Describer describer, const SourceFile* const source)
    {
        ...
        if (getNode != nullptr)
        {
            ...
            const auto body = getNode->GetChild(static_cast<int>(ChildCode::Body));
            if (body == nullptr)
                PushException(new AccessorBodyException(getNode->Token().Index(), source));

            get = new MethodFunction(std::string(get_indexer_name), actualDescriber, creationType, body);
        ...
        if (setNode != nullptr)
        {
            ...
            const auto body = setNode->GetChild(static_cast<int>(ChildCode::Body));
            if (body == nullptr)
                PushException(new AccessorBodyException(setNode->Token().Index(), source));

            set = new VoidFunction(std::string(set_indexer_name), actualDescriber, body);
    ...
    void CreateIndexer(const IParseNode* const indexerNode, IUserDefinedType* const dataType)
    {
        ...
        {
            const auto getBody = getNode->GetChild(static_cast<int>(ChildCode::Body));
            const auto setBody = getNode->GetChild(static_cast<int>(ChildCode::Body));
