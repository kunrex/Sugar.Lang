...
namespace Tokens
{
    ...
    Token Operator::Unary(const unsigned long index, const SyntaxKind syntaxKind, const bool leftAssociative, const int precedence)
    {
        return { index, TokenType::UnaryOperator, syntaxKind, EncodeMetadata(leftAssociative, precedence), 0l };
    }

    Token Operator::Binary(const unsigned long index, const SyntaxKind syntaxKind, const int precedence)
    {
        return { index, TokenType::BinaryOperator, syntaxKind, EncodeMetadata(true, precedence), 0l };
    }

    Token Operator::Assignment(const unsigned long index, const SyntaxKind syntaxKind)
    {
        return { index, TokenType::BinaryOperator, syntaxKind, 13, 0l };
    }
    ...
    Token Operator::As(const unsigned long i) { return Binary(i, SyntaxKind::As, 1); }
